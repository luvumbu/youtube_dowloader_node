...
function createButtons() {
  ...
  bar.innerHTML = `
    ...
    <button id="ytdl-btn-flow" class="ytdl-fab ytdl-fab-flow" title="Ajouter a Mon Flow">+F</button>
  ...
  document.getElementById('ytdl-btn-flow').addEventListener('click', quickAddToFlow);
...
// ========== QUICK ADD TO FLOW ==========
async function quickAddToFlow() {
  const url = window.location.href;
  const btn = document.getElementById('ytdl-btn-flow');
  if (!btn) return;
  btn.textContent = '...';
  ...
  try {
    ...
    if (!info.success) { btn.textContent = '!'; return; }
  ...
  btn.style.opacity = '1';
  setTimeout(() => { btn.textContent = '+F'; btn.style.background = '#9C27B0'; }, 2500);
}

// ========== PANNEAU OPTIONS ==========
async function loadFlowPlaylistsPanel() {
  try {
    ...
    const sel = document.getElementById('ytdlFlowPlaylist');
    if (sel) {
      sel.innerHTML = '<option value="">Sans playlist</option>' + allPls.map(p => '<option value="' + p + '">' + p + '</option>').join('');
    }
  } catch (e) {}
}
...
function createPanel() {
    ...
    <div class="ytdl-body">
      ...
      <div style="margin-top:8px;padding-top:8px;border-top:1px solid rgba(255,255,255,0.1);">
        <div style="display:flex;gap:4px;margin-bottom:6px;">
          <select id="ytdlFlowPlaylist" style="flex:1;padding:6px 8px;background:#111;border:1px solid #9C27B0;border-radius:6px;color:#fff;font-size:11px;">
            <option value="">Sans playlist</option>
          </select>
          <button id="ytdlFlowNewBtn" style="background:#9C27B0;color:#fff;border:none;width:26px;height:26px;border-radius:6px;font-size:16px;cursor:pointer;font-weight:700;">+</button>
        </div>
        <div id="ytdlFlowNewRow" style="display:none;gap:4px;margin-bottom:6px;">
          <input type="text" id="ytdlFlowNewName" placeholder="Nom..." style="flex:1;padding:6px 8px;background:#111;border:1px solid #9C27B0;border-radius:6px;color:#fff;font-size:11px;">
          <button id="ytdlFlowCreateBtn" style="background:#9C27B0;color:#fff;border:none;padding:6px 10px;border-radius:6px;font-size:11px;cursor:pointer;">OK</button>
        </div>
        <button class="ytdl-btn" id="ytdlBtnFlow" style="background:#9C27B0;color:#fff;margin-bottom:0;">+ Ajouter a Mon Flow</button>
      </div>
  ...
  // Mon Flow buttons
  document.getElementById('ytdlFlowNewBtn').addEventListener('click', () => {
    const row = document.getElementById('ytdlFlowNewRow');
    row.style.display = row.style.display === 'none' ? 'flex' : 'none';
    if (row.style.display === 'flex') document.getElementById('ytdlFlowNewName').focus();
  });

  document.getElementById('ytdlFlowCreateBtn').addEventListener('click', async () => {
    const name = document.getElementById('ytdlFlowNewName').value.trim();
    if (!name) return;
    await fetch(API + '/flow', { method: 'POST', headers: { 'Content-Type': 'application/x-www-form-urlencoded' }, body: 'action=create_playlist&name=' + encodeURIComponent(name) });
    document.getElementById('ytdlFlowNewName').value = '';
    document.getElementById('ytdlFlowNewRow').style.display = 'none';
    await loadFlowPlaylistsPanel();
    document.getElementById('ytdlFlowPlaylist').value = name;
    const s = document.getElementById('ytdlStatus');
    if (s) { s.textContent = 'Playlist "' + name + '" creee !'; s.className = 'ytdl-status ok'; }
  });

  document.getElementById('ytdlFlowNewName').addEventListener('keydown', (e) => {
    if (e.key === 'Enter') document.getElementById('ytdlFlowCreateBtn').click();
  });

  document.getElementById('ytdlBtnFlow').addEventListener('click', async () => {
    const url = window.location.href;
    const btn = document.getElementById('ytdlBtnFlow');
    const status = document.getElementById('ytdlStatus');
    ...
    try {
      ...
      const info = await infoResp.json();
      if (!info.success) { if (status) { status.textContent = 'Erreur'; status.className = 'ytdl-status error'; } return; }

      const playlist = document.getElementById('ytdlFlowPlaylist').value;
      const resp = await fetch(API + '/flow', {
        method: 'POST', headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
        body: 'action=add&url=' + encodeURIComponent(url)
          + '&title=' + encodeURIComponent(info.title || '')
      ...
      if (data.duplicate) {
        btn.textContent = 'Deja dans Mon Flow';
        if (status) { status.textContent = 'Deja dans Mon Flow'; status.className = 'ytdl-status ok'; }
      } else {
        const pl = playlist ? ' (' + playlist + ')' : '';
        btn.textContent = 'Ajoute !';
        if (status) { status.textContent = 'Ajoute a Mon Flow' + pl; status.className = 'ytdl-status ok'; }
      }
    } catch (e) {
      btn.textContent = 'Erreur';
      if (status) { status.textContent = 'Erreur: ' + e.message; status.className = 'ytdl-status error'; }
    }
    setTimeout(() => { btn.textContent = '+ Ajouter a Mon Flow'; btn.disabled = false; }, 3000);
  });

  loadFlowPlaylistsPanel();
  updatePanelOptions();
  loadSavedPrefs();
}
...
function createPlaylistBanner() {
  ...
  banner.innerHTML = `
    <div class="ytdl-pb-top" id="ytdlPbToggle">
      ...
      <div class="ytdl-pb-right">
        <button id="ytdl-pb-flow" class="ytdl-pb-flow" onclick="event.stopPropagation()">+ Mon Flow</button>
  ...
  document.getElementById('ytdl-pb-flow').addEventListener('click', async () => {
    const btn = document.getElementById('ytdl-pb-flow');
    const filtered = getFilteredVideos();
    if (filtered.length === 0) return;
    btn.textContent = 'Ajout en cours...';
    btn.disabled = true;

    // Recuperer le nom de la playlist YouTube
    ...
    // Creer la playlist dans Mon Flow
    try {
      await fetch(API + '/flow', {
        method: 'POST', headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
        body: 'action=create_playlist&name=' + encodeURIComponent(playlistName)
      });
    } catch (e) {}

    // Ajouter tous les elements dans cette playlist
    ...
    try {
      ...
      const data = await resp.json();
      btn.textContent = data.added + ' ajoute(s) a "' + playlistName + '" !';
      btn.style.background = '#4CAF50';
    } catch (e) {
      btn.textContent = 'Erreur';
    }
    setTimeout(() => { btn.textContent = '+ Mon Flow'; btn.style.background = '#9C27B0'; btn.disabled = false; }, 3000);
  });
